                                                                    ...
                                                                    <div class="form-group">
                                                                        <div class="controls">
                                                                            <select name="end_interval[{{ $day }}][]" id="{{$day}}_end"  class="form-control">
                                                                                ...
                                                                                foreach(@$timeslot as $slot){
                                                                                    $selected = "";
                                                                                    if($slot->time_name == 9){$selected = 'selected="selected"';}
                                                                                    echo "<option data-dt='".ltrim(date('H', strtotime($slot->time_name)), '0')."' value='$slot->time_name' $selected  >$slot->time_name</option>";
                                                                                }
                                                                                ?>
                                                                            </select>
                                                                        </div>
                                                                                    ...
                                                                                    <select name="start_interval[{{ $val->day }}][]" id="{{$val->day}}_start"  class="form-control">
                                                                                        ...
                                                                                        foreach($timeslot as $slot){
                                                                                            if($slot->time_value!='11:55 AM'){
                                                                                                $selected = "";
                                                                                                if($value->start_hour ==  $slot->time_name){$selected = 'selected="selected"';}
                                                                            ...
                                                                            <div class="form-group">
                                                                                <div class="controls">
                                                                                    <select name="end_interval[{{ $val->day }}][]" id="{{$val->day}}_end"  class="form-control">
                                                                                        ...
                                                                                        foreach(@$timeslot as $slot){
                                                                                            $selected = "";
                                                                                            if($value->end_hour ==  $slot->time_name){$selected = 'selected="selected"';}
                                                                                            echo "<option data-dt='".ltrim(date('H', strtotime($slot->time_name)), '0')."' value='$slot->time_name' $selected  >$slot->time_name</option>";
                                                                                        }
                                                                                        ?>
                                                                                    </select>
...
@section('scripts')
    ...
    <script>
        ...
        function add_time(e, day){
            ...
            var htm = '<tr class="'+day+'_">' +
                ...
                '               <select name="end_interval['+day+'][]" id="'+day+'_end" class="form-control">' +
                @foreach($timeslot as $slot)
                    '<option data-dt="" {{$slot->time_name == 9? 'selected':''}} value="{{$slot->time_name}}">{{$slot->time_name}}</option>'+
                @endforeach
                '                </select>'+
                '            </div>'+
                '        </div>'+
    ...
    <script async='' defer='defer' src='https://maps.googleapis.com/maps/api/js?key=AIzaSyCAwX5DJjr0fC7_vp6_WVO6Ut16hXTuK1g&amp;libraries=places&amp;language=en&amp;region=AU&amp;callback=initMap' type="text/javascript"></script>
