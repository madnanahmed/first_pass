                            ...
                            <table class="table table-bordered table-striped" width="100%">
                                ...
                                @if(isset($appointments) && count($appointments) > 0 )
                                    @foreach($appointments as $appointment)
                                        <tr class="@if($appointment->time_slot == '') bg-danger text-white @endif" @if($appointment->time_slot == '') data-toggle="tooltip" data-title="Instructor can't approve a lesson if time is missed, please add schedule time" @endif>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div class="m-r-10">
                                                        ...
                                                        @else
                                                            <img src="{{ url('assets/images/users/'.$appointment->avatar) }}" alt="user" class="img-circle rounded-circle" width="60">
                                                        @endif
                                                    </div>
                                                    <div class="">
                                                        <h4 class="m-b-0 font-16">{{$appointment->name}} {{$appointment->lname}}</h4>
                                                        <span>{{$appointment->email}}</span><br>
                                                        <span>{{$appointment->phone}}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>{{$appointment->schedule_date . ' '.$appointment->time_slot}}</td>
                                            <td>{{$appointment->type}}</td>
                                            <td>{{$appointment->status}}</td>
                                            <td><a class="btn btn-xs btn-info" href="javascript:void(0)" onclick="AppointmentDetailModal(this)" data-appointment-id="{{$appointment->id}}" data-user-type="{{$appointment->type}}"><i class="fa fa-eye"></i> View Detail</a></td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                            ...
                            <div class="table-responsive" >
                                <table class="table table-bordered table-striped">
                                    ...
                                    @foreach($instructors as $inst)
                                        ...
                                        $vehicle_image = \App\UserMeta::where('user_id', $inst->instructor_id)->value('vehicle_image');
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $completed }}</td>
                                            <td>{{ $total }}</td>
                                            <td><a data-fancybox="gallery" href="{{ asset('assets/images/vehicle_images/'.$vehicle_image) }}"><img src="{{ asset('assets/images/vehicle_images/'.$vehicle_image) }}" alt="" height="60"></a></td>
                                            <td>{{ $rating->review }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <a href="{{ url('search/instructors/'.$inst->id) }}" type="button" class="btn btn-warning"> BOOK NOW </a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
