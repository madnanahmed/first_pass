...
class GuestController extends Controller
{
    ...
    function search(Request $request){
        ...
        if($request->search_type == 1){
            ...
            /*save search*/
            $data = [
                'time_slot_test_time'=> $request->time_slot_test_time,
                'time_slot' => $request->time_slot,
                'lesson_hour' => $request->hour,
    ...
    function book_online_book($search_id, $instructor_id){
        ...
        return view('search.book', compact('test_locations','user_working_time','instructor', 'search', 'search_id', 'region', 'test_package'));
    }
    ...
    function get_slots(Request $request){
        ...
        if($instructor) {
            ...
            if ($get_times) {
                ...
                if (is_array($av_time)) {
                    $html = "<select name='time_slot' onchange='check_availiblity(this)'><option value=''>Select lesson time</option>";
                    foreach ($av_time as $v) {
                        ...
                        foreach ($slots as $i => $slot) {
                            if (in_array($slot['start'].'-'.$slot['end'], $boobkly_appts)) {
                                $html .= '<option disabled value="' . $slot['start'].'-'.$slot['end'] . '">' . $slot['start'].'-'.$slot['end'] . 'Booked</option>';
                            } else {
                                $html .= '<option data-start="'.strtotime($slot['start']).'" data-end="'.strtotime($slot['end']).'" value="' . $slot['start'].'-'.$slot['end'] . '" >' . $slot['start'].'-'.$slot['end'] . '</option>';
                            }
                        }
                    }
                }
    ...
    function get_slots1(Request $request){
        ...
        if($get_times){
            ...
            if (is_array($av_time)) {
                $html = "<select name='time_slot_test_time' onchange='check_availiblity1(this)'><option value=''>Select test lesson time</option>";
                foreach ($av_time as $v) {
                    $start_time = $v->start_hour . ':' . $v->start_min;
                    $end_time = $v->end_hour . ':' . $v->end_min;
                    $start_carb = Carbon::parse($start_time)->addMinutes($instructor->lesson_travel_time)->format('H:i');
      ...
      //  try{
            ...
            if($search) {
                ...
                if($stripe_key!=''){
                    ...
                    if(isset($sch->time_slot_test_time))
                    {
                        $time_slot_test_time= $sch->time_slot_test_time;
                    }
                    ...
                    if (is_numeric($amount)) {
                        ...
                        foreach ($step_2 as $package) {
                            ...
                            if ($package == 'test') {
                                ...
                                $appointmetObj->time_slot = $time_slot_test_time;
                            ...
                            if ($charge['paid'] == 1) {
                                ...
                                if ($payment_response->save()) {
                                    ...
                                    if ($instructor) {
                                        ...
                                        if($time_slot!=''){
                                            $schedule_date = $schedule_date.' '. $time_slot;
                                        }
